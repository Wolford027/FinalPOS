...
const International = () => {
    ...
    useEffect(() => {
        const fetchProds = async () => {
            setLoading(true);
            try {
                const querySnapshot = await getDocs(collection(db, 'International'));
                const prodList = [];
                const imgList = [];
    ...
    const updateImg = async (prodId, imgURL) => {
        try {
            const prodRef = doc(db, 'International', prodId);
    ...
    const saveChange = async () => {
        if (selectedProd) {
            try {
                const prodRef = doc(db, 'International', selectedProd.id);
                await updateDoc(prodRef, {
                    Name: newName,
                    Price: newPrice,
                });
