...
const AllDayBreakfast = () => {
    ...
    useEffect(() => {
        const fetchProds = async () => {
            setLoading(true);
            try {
                const querySnapshot = await getDocs(collection(db, 'AllDayBreakfast'));
                const prodList = [];
                const imgList = [];
    ...
    const updateImg = async (prodId, imgURL) => {
        try {
            const prodRef = doc(db, 'AllDayBreakfast', prodId);
    ...
    const saveChange = async () => {
        if (selectedProd) {
            try {
                const prodRef = doc(db, 'AllDayBreakfast', selectedProd.id);
                await updateDoc(prodRef, {
                    Name: newName,
                    Price: newPrice,
                });
