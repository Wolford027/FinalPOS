...
const BoodleFight = () => {
    ...
    useEffect(() => {
        const fetchProds = async () => {
            setLoading(true);
            try {
                const querySnapshot = await getDocs(collection(db, 'BoodleFight'));
                const prodList = [];
                const imgList = [];
    ...
    const updateImg = async (prodId, imgURL) => {
        try {
            const prodRef = doc(db, 'BoodleFight', prodId);
    ...
    const saveChange = async () => {
        if (selectedProd) {
            try {
                const prodRef = doc(db, 'BoodleFight', selectedProd.id);
                await updateDoc(prodRef, {
                    Name: newName,
                    Price: newPrice,
                });
