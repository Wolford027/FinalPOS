...
const KTVRooms = () => {
    ...
    useEffect(() => {
        const fetchProds = async () => {
            setLoading(true);
            try {
                const querySnapshot = await getDocs(collection(db, 'KTVRooms'));
                const prodList = [];
                const imgList = [];
    ...
    const updateImg = async (prodId, imgURL) => {
        try {
            const prodRef = doc(db, 'KTVRooms', prodId);
    ...
    const saveChange = async () => {
        if (selectedProd) {
            try {
                const prodRef = doc(db, 'KTVRooms', selectedProd.id);
                await updateDoc(prodRef, {
                    Name: newName,
                    Price: newPrice,
                });
